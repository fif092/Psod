...
program sphericaloverdensity
  ...
  real(8)::seuil=600.d0,seuil2=800.d0
  ...
  real(4), dimension(3)   :: mintmp,maxtmp
  real(4), allocatable    :: cdm(:,:)
...
!  do kk=7,nslice-1
  ...
  allocate(mp(1:npart),xsort(1:npart),xsortsave(1:npart))
...
!  deallocate(vel,id)
  ...
  enddo
  if(nx==0)then
     nx=idnint(dble(npart)**(1.d0/3.d0))
  endif
  ...
  xxmin=0.d0;  xxmax=1.d0;  yymin=0.d0 ;yymax=1.d0 ; zzmin=0.d0 ; zzmax=1.d0
  xmin=0.d0;  xmax=1.d0 ;ymin=0.d0 ; ymax=1.d0 ; zmin=0.d0; zmax=1.d0
  nxcic=2*nx; nycic=2*ny ; nzcic=2*nz
  local_nx=nxcic;  local_ny=nycic;  local_nz=nzcic
  ixzonecic=0;iyzonecic=0;izzonecic=0
  nxzonecic=1;nyzonecic=1;nzzonecic=1
  periodic=.true.
  call cic(npart,xxmin,xxmax,yymin,yymax,zzmin,zzmax,xmin,xmax,ymin,ymax,zmin,zmax,local_nx,local_ny,local_nz,nxcic,nycic,nzcic,ixzonecic,iyzonecic,izzonecic,nxzonecic,nyzonecic,nzzonecic,periodic)
  print*,'minmeanmax(cube)',minval(cube),sum(cube(0:nxcic-1,0:nycic-1,0:nzcic-1))/(1.d0*nxcic*nycic*nzcic),maxval(cube)

  call cicm1(npart,xxmin,xxmax,yymin,yymax,zzmin,zzmax,nxcic,nycic,nzcic)
  print*,'minmeanmax(rho)',minval(rho),sum(rho)/(1.d0*npart),maxval(rho)

  open(9997,file='fort.9997',form='unformatted')
  write(9997)npart
  write(9997)idpreal8
  close(9997)
  
  open(9998,file='fort.9998',form='unformatted')
  write(9998)npart
  write(9998)rho
  close(9998)
  ...
  !------------------------------------------
!  call tri_ngp(xsort,isort,npart,nxcic,nycic,nzcic,npart_new)
!  stop
  ...
  open(9996,file='fort.9996',form='unformatted')
  write(9996)npart
  write(9996)xsortsave
  close(9996)
  ...
  call spherover(nchar,npart,nx,ny,nz,npart_new,ncut,seuil,seuil2,Mmin,scale)
  write(*,*) 'done'
...
subroutine tri_cic(pdens,isort,npart,nx,ny,nz,npart_new)
  ...
  i=1
  do while (pdens(i) < 1.d0/2.d0) !the 2 here can be increase up to delta or a fraction of delta I think
     i=i+1
  end do
  npart_new=i
  write(*,*)'npart_active=',npart_new
...
subroutine cic(npart,xxmin,xxmax,yymin,yymax,zzmin,zzmax,xmin,xmax,ymin,ymax,zmin,zmax,local_nx,local_ny,local_nz,nx,ny,nz,ixzone,iyzone,izzone,nxzone,nyzone,nzzone,periodic)
...
if(periodic)then
   ...
   do i=1,npart
      ...
      ok_part=(((x(i,1)>=xmin  .and.x(i,1)<=xmax).or.        &
           &    (x(i,1)>=xmin+1.d0.and.x(i,1)<=1.d0  ).or.        &
           &    (x(i,1)>=0.    .and.x(i,1)<=xmax-1.d0))   .and.&
           &   ((x(i,2)>=ymin  .and.x(i,2)<=ymax).or.        &
           &    (x(i,2)>=ymin+1.d0.and.x(i,2)<=1.d0  ).or.        &
           &    (x(i,2)>=0.    .and.x(i,2)<=ymax-1.d0))   .and.&
           &   ((x(i,3)>=zmin  .and.x(i,3)<=zmax).or.&
           &    (x(i,3)>=zmin+1.d0.and.x(i,3)<=1.d0  ).or.&
           &    (x(i,3)>=0.    .and.x(i,3)<=zmax-1.d0)))
      ...
      if(ok_part)then
       mtmp=mtmp+mp(i)
         ...
         !periodic BC (assumes CIC + utile zone is within [0,1[+ serial) -> to be improved for parallel + if not in [0,1[
         !if(ix+ixzone*local_nx==0.and.ixzone==nxzone-1)ix=local_nx 
         if(ixp1==nx.and.ixzone==0)ixp1=0 
         if(ix==nx.and.ixzone==0)ix=0 
         
         !if(iy+iyzone*local_ny==0.and.iyzone==nyzone-1)iy=local_ny 
         if(iyp1==ny.and.iyzone==0)iyp1=0 
         if(iy==ny.and.iyzone==0)iy=0 

       !  if(iz+izzone*local_nz==0.and.izzone==nzzone-1)iz=local_nz 
         if(izp1==nz.and.izzone==0)izp1=0 
         if(iz==nz.and.izzone==0)iz=0 
...
open(9999,file='fort.9999',form='unformatted')
write(9999)local_nx,local_ny,local_nz
write(9999)cube!(0:local_nx-1,0:local_ny-1,0:local_nz-1)
close(9999)
end subroutine cic


subroutine cicm1(npart,xxmin,xxmax,yymin,yymax,zzmin,zzmax,nx,ny,nz)
...
do i=1,npart
   ...
   if(ok_part)then
      ...
      rho(i)= rho(i)+cube(ix  ,iy  ,iz  )*dex*dey*dez
      rho(i)= rho(i)+cube(ix  ,iyp1,iz  )*dex*ddy*dez
      rho(i)= rho(i)+cube(ixp1,iy  ,iz  )*ddx*dey*dez
      rho(i)= rho(i)+cube(ixp1,iyp1,iz  )*ddx*ddy*dez
      rho(i)= rho(i)+cube(ix  ,iy  ,izp1)*dex*dey*ddz
      rho(i)= rho(i)+cube(ix  ,iyp1,izp1)*dex*ddy*ddz
      rho(i)= rho(i)+cube(ixp1,iy  ,izp1)*ddx*dey*ddz
      rho(i)= rho(i)+cube(ixp1,iyp1,izp1)*ddx*ddy*ddz
...
!---------------------------------------------------------------------------
subroutine spherover(nchar,npart,nx,ny,nz,npart_new,ncut,seuil,seuil2,Mmin,scale)
!---------------------------------------------------------------------------
  use modvar
  implicit none
  ...
  !real(8),dimension(1:npart)::xsort
  ...
  integer,dimension(:),allocatable::structure,strct,indx_ngp,isort_ngp,allpart,masses
  ...
  character*80::nomf,nomf2,filestrct,fileamas,fileprofile,filestrctfof
  integer::count_no_halo,gap,namas
  real(8)::frac=1.d0 !!! Control when to stop the loop on particles. Added by Yann ! a diminuer ?

  real(8),dimension(1:nbins)::rad_bins
  integer,dimension(1:nbins)::mass_bins

  allocate(structure(1:npart))
  allocate(strct(1:npart))
  allocate(indx_ngp(1:npart))
  allocate(isort_ngp(1:npart))
  allocate(masses(1:npart),allpart(1:npart))
  structure = 0
  taillex   = dble(nx)
  tailley   = dble(ny)
  taillez   = dble(nz)
  rayon_min = sqrt(3.d0)*scale/(2.d0*nx)  !!!a bit large no???? RY
  ...
  filestrctfof = 'sod_'//trim(nchar)//'_strct_00000'
  gap=0
  namas=0
...
!        do while(dt.gt.dtmax.and.niter.le.100)
           ...
           do while(indx_ngp(l).eq.indc.and.l.lt.npart)  !I am wondering if the cells are not too large RY...
              !write(*,*) 'entered loop'
              !x(isort(ipart) ?
...
!           if(masse.eq.470.or.masse.eq.323)then
!              write(*,'(A10,I8,3F10.3,I8,F10.3)')'THIS ONE',masse,xc,yc,zc,n_max,xsortsave(isort(ipart))
!           endif
