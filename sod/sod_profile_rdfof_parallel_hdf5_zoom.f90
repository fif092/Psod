module modvar
  ...
  real,dimension(:),allocatable::potf   !potential
  real,dimension(:,:),allocatable::forf   !force
...
program sphericaloverdensity
  ...
  !+ test version ultra original sod + test profile + selection moins large rayon_min + test occupation memoire etc... + integer8+WRITEHDF5 option
  !---------------------------------------------------------------------
  !F.Tornatore, 08/11//2024
  !Adapt to zoom run
  !---------------------------------------------------------------------
  ...
  integer::ncut=4,nxbuffer=0,Nsel=4000000  !Nsel --> huge integer that is never used
  ...
  real(8) :: dx,dy,dz
  real(8) :: mmin_part, mmax_part  ! FT mass min and max found in the sim
  integer :: level_min, level_max, level_max_global, level_min_global  ! FT level corresponding to mminpart and mmax_part
  real(8) :: xzmin, xzmax ,xzmin_global, xzmax_global
  ! IO variables
  integer   :: uinout=20
  logical   :: found
...
!  integer::mpierr
  ...
  !CIC or POT logical variables to sort particles according to CIC density or pot
  logical:: dopot=.false. 

  logical:: docic=.true. !warning dopot is higher priority than docic
  ...
  ! Count the total number of particles in the simulation !!!WARNING SHOULD BE LONG/INTEGER8???? 
  nthalo=0
  if(procID==0)then
     do kk=0,ncpu-1
  ...
  islast=.false.
  Call h5readcommonmetadata(file_id, islast,inforamses=inforamses)
  
  ! open the root group
  grname = '/metadata/'
  Call hdf5_open_group(file_id,grname, gr_id)

  dname='npart_simulation'

  Call hdf5_read_data(gr_id,dname,nparttotal)   
...
! Loop on the utile zones (cube of number nzone) that will be analyzed 
  do nzone=firstp,lastp   !nzone is the 1D index of the zone that is currently analyzed by the process
     
     !-------------------------------------------------------------------------------!
     !PARALLEL READING OF CUBE FILES AND SELECTION OF PARTICLES (WORKING+BUFFER ZONE)!
     !-------------------------------------------------------------------------------!
     
     ! Move nzone to a 3D index
     ...
     !number of cells in the zone along x direction
     nxzone=nx/nxcpu  

     !range of index of cubes to read --> encompass the index of the 26 neigbouhring cube
     ixmin=ixzone-1
     ixmax=ixzone+1
     iymin=iyzone-1
     iymax=iyzone+1
     ...
     !Compute cube list to read = retrieve 1D index of the cube to read
     kk=0
     do i=ixmin,ixmax 
        do j=iymin,iymax
           do k=izmin,izmax
     ...
     !Check validity of buffer zone  !ncut is the estimated maximum half length of a halo in grid unit
     !usefull to make sure that no halo is cut by the buffer zone --> conservative choice: ncut high
     ...
     !distances between the center of the full zone (working+buffer) and the limits
     rxsmall=(0.5d0+real(nxbuffer,kind=8)/real(nxzone,kind=8))/real(nxcpu,kind=8)  !0.5 (boxe center) + buffer zone size
     rysmall=(0.5d0+real(nxbuffer,kind=8)/real(nxzone,kind=8))/real(nycpu,kind=8)  !in units of nxzone == unit of boxe
     rzsmall=(0.5d0+real(nxbuffer,kind=8)/real(nxzone,kind=8))/real(nzcpu,kind=8)  !divided by nxcpu to scale down the region
     
     rxlarge=1.d0-rxsmall !rxlarge plays the same role than rxsmall but for the buffer zone in the BCs
     ...
     if(IOGROUPSIZE>0) then
        ...
        if(potential)allocate(potf(1:npart))
        if(force)allocate(forf(1:ndim,1:npart))
           ...
           do i=1,nloc
              ...
              if ((dx.lt.rxsmall.or.dx.gt.rxlarge).and.&
                 ...
                 do j=1,3
                    ...
                    if(force)forf(j,ifillpart)=for(j,i)
                 enddo
                 mpf(ifillpart)=1  !non zoom sims
                 idf(ifillpart)=idtmp(i)
                 if(potential)potf(ifillpart)=pot(i)                 
              endif
        ...
        if(procID==0) then 
           print*,'WARNING GATHERWRITE HARDCODED IN MODEXTRACT'
        endif

        Call extract_cuboid()
        
!        call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
!        print*,'Read done',procID
!        call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
!        stop
        ...
        if (procID==0)write(*,*)"npartini = ", npartini

        if(procID ==0)print*,procID,'npart read',npartini ! npartini == total number of part in the sim
        !each proc reads all the particles --> could be improved for large sim ?
        
        !Select only particles in region=> TODO to be improved for memory
        !count number of particles
        npart=0
        mmin_part = 10000.0 ! FT initialize mminpart and mmaxpart
        mmax_part = 0.0     ! FT
        do i=1,npartini !loop first to get number of particles in the working region
           dx=abs(pos(1,i)-xc)
           dy=abs(pos(2,i)-yc)
           dz=abs(pos(3,i)-zc)
           if ((dx.lt.rxsmall.or.dx.gt.rxlarge).and.&
                (dy.lt.rysmall.or.dy.gt.rylarge).and.&
                (dz.lt.rzsmall.or.dz.gt.rzlarge)) then
                npart=npart+1      !select particles in the the zone analyzed by the process (center + buffer)
           endif
        end do
        

        if(procID==0)then
           print*,procID, 'npart count region=',npart !nb of particle in the working region analyzed by the proc
           print*,'rxsmall',rxsmall
           print*,'rxlarge',rxlarge
           print*,'center',xc,yc,zc
        endif
        
        allocate(posf(1:ndim,1:npart),velf(1:ndim,1:npart)) ! allocate arrays
        allocate(idf(1:npart))
        allocate(mpf(1:npart))
        if(potential)allocate(potf(1:npart))
        if(force)allocate(forf(1:ndim,1:npart))
        ...
        ifillpart=0
        do i=1,npartini  ! second loop to get pos,vel, id and mass of particles in the working region
           dx=abs(pos(1,i)-xc)
           dy=abs(pos(2,i)-yc)
           dz=abs(pos(3,i)-zc)
           if ((dx.lt.rxsmall.or.dx.gt.rxlarge).and.&
              ...
              do j=1,3
                 ...
                 if(force)forf(j,ifillpart)=for(j,i)
              enddo
              mpf(ifillpart)=mass(i)    ! FT previous: mpf(ifillpart)=1 
              
              if (mass(i) .gt. mmax_part) then  ! FT get mmax_part
                 mmax_part = mass(i)
              end if

              if (mass(i) .lt. mmin_part) then  ! FT get mmin_part
                 mmin_part = mass(i)
              end if
        ...
        level_max = int(log((1.0 / mmin_part)**(1.0 / 3.0)) / log(2.0)) ! FT compute level_max
        level_min = int(log((1.0 / mmax_part)**(1.0 / 3.0)) / log(2.0)) ! FT compute level_min
        
        ! FT
        xzmin=1.0
        xzmax=0.0
        do i=1,npartini  ! third loop to get limit of the zoomed region
           dx=abs(pos(1,i)-xc)
           dy=abs(pos(2,i)-yc)
           dz=abs(pos(3,i)-zc)
           if ((dx.lt.rxsmall.or.dx.gt.rxlarge).and.&
                (dy.lt.rysmall.or.dy.gt.rylarge).and.&
                (dz.lt.rzsmall.or.dz.gt.rzlarge)) then
              if (mass(i)==mmin_part) then
                 do j=1,3                           !FT loop to get xzmax and xzmin
                    if (pos(j,i).gt.xzmax) then
                       xzmax=pos(j,i)
                    endif
                    if (pos(j,i).lt.xzmin) then
                       xzmin=pos(j,i)
                    endif
                 enddo
              endif
        ...
        call MPI_ALLREDUCE(xzmin, xzmin_global , 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD, mpierr) ! FT get min among all proc
        call MPI_ALLREDUCE(xzmax, xzmax_global , 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD, mpierr) ! FT get max among all proc
        call MPI_ALLREDUCE(level_min, level_min_global , 1, MPI_INTEGER, MPI_MIN, MPI_COMM_WORLD, mpierr) ! FT get min among all proc
        call MPI_ALLREDUCE(level_max, level_max_global , 1, MPI_INTEGER, MPI_MAX, MPI_COMM_WORLD, mpierr) ! FT get max among all proc

        if (procID==0) write(*,*) ""
        if (procID==0) write(*,*) "Level min is: ", level_min_global    ! FT
        if (procID==0) write(*,*) "Level max is: ", level_max_global
        if (procID==0) write(*,*) "Min of zone to explore is: ", xzmin_global
        if (procID==0) write(*,*) "Max of zone to explore is: ", xzmax_global
        if (procID==0) write(*,*) ""

        deallocate(pos,vel,id,mass)
        if(potential)deallocate(pot)
        if(force)deallocate(for)
#endif
     ...
     if (docic)then
        ...
        !previous: conversion here to mass in sim units --> not usefull anymore in zoom version as masses read in pfof cubes are already in
        !sim units
        ...
        nxcic=2*nx; nycic=2*ny ; nzcic=2*nz  ! mass assignation on a grid 2 times finer than nx 
        ...
        if(procID==0)print*,procid,'minmeanmax(cube)',minval(cube),sum(cube)/&
        &(1.d0*(local_nx+1)*(local_ny+1)*(local_nz+1)),maxval(cube)
        ...
        mpf=mpf*real((2**level_max_global)**3,kind=8) ! FT with this conversion, level max particles have mpf=1.0, next level mpf=8.0
                                               ! then mpf=64.0 and so one
    endif                                     
     ...
     Mmin=dble(nmin)  !min is 100 particle of finest level
     if (procID==0)write(*,*) "Minimum halo mass is: ", Mmin
     ...
     if(dopot)then
        if(procID==0)write(*,*) 'Sorting particles by potential values'
        nxpot=nx !nxpot unused because pot already computed but we keep similar arguments as tri_cic
        nypot=ny !same
        nzpot=nz !same
        call tri_pot(xsort,isort,npart,nxpot,nypot,nzpot,npart_new,seuilpot)
     else
        if(docic)then
     ...
     xxmin=xmin;  xxmax=xmax;  yymin=ymin ;yymax=ymax ; zzmin=zmin ; zzmax=zmax
     local_nx=(nxzone+2*nxbuffer);  local_ny=(nxzone+2*nxbuffer);  local_nz=(nxzone+2*nxbuffer)
     call spherover(nchar,npart,nx,ny,nz,&
          ...
          xxmin,xxmax,yymin,yymax,zzmin,zzmax,local_nx,local_ny,local_nz,periodic,level_max_global,xzmin_global&
          ,xzmax_global)

     deallocate(posf,velf,idf)
     deallocate(mpf)
     if(force)deallocate(forf)
...
contains
  subroutine read_params
    ...
    use modparameters
      ...
      LOGICAL       :: bad, ok
      character*200::repositoryonly,simuname,filepart
      integer:: nsnap
      nsnap=0
      simuname='test'
      ...
      if (n < 7) then
         ...
         print *, '            [-fil  particle filename (without extension)'
         ...
         print *, '            [-sim simuname]     (optional:simulation name)'
         print *, '            [-snp nsnap] (optional: snapshot number)'
         print *, '            [-hlp]          (optional)'
         endif
         call MPI_FINALIZE(mpierr)
         stop
      ...
      repository=trim(repositoryonly)//'/'//trim(filepart)

      root=repository

#ifdef READHDF5  
      param%input_path=repositoryonly
      param%part_input_file=filepart
      param%simulation_name=simuname
      param%mmin=nmin
      param%mmax=2000000000 !Any very high value that is never used
      param%do_read_potential=potential
      param%do_read_gravitational_field=force
      param%do_unbinding=.false.
      param%do_subhalo=.false.
      param%do_timings=.false.
      param%percolation_length=0

      param%matter_overdensity_threshold=seuil      !new
      param%grpsize=0
      param%gatherread_factor=1 !WARNING SET TO 1 (ISC 19/07/2022)
      param%snapshot=nsnap         
      param%gatherwrite_factor=1 !WARNING SET TO 1 (ISC 19/07/2022)
      param%do_skip_star=.false.      
      param%do_skip_metal=.false.           
      param%do_read_from_cube=.true.       
      param%do_sod=.true.                   !replace do_fof
      param%do_write_cube=.false.              
      param%do_sort_cube=.false.               
      param%code_index='fof' 
      param%info_input_file=trim(root)//'_'//'00000'//'.h5'       

      !WARNING: ISC 19/07/2022
      !if(param%gatherread_factor.ne.2) then
         print*,'WARNING MAKE SURE GATHER WRITE IS THE SAME IN MODEXTRACT AND HERE=>TO BE IMPROVED',param%gatherread_factor
      !   stop
      !endif
...
subroutine tri_ngp(pdens,isort,npart,nx,ny,nz,npart_new)
  ...
  call MPI_COMM_SIZE(MPI_COMM_WORLD,procNB,mpierr)
  call MPI_COMM_RANK(MPI_COMM_WORLD,procID,mpierr)
  if(procID==0)print*,'WARNING THIS OLD ROUTINE'
  if(procID==0)print*,'IS LOOSELY PARALLELIZED BECAUSE SOME ARRAY ARE STILL GLOBAL'
  ngx=nx
  ngy=ny
  ngz=nz
...
subroutine tri_cic(pdens,isort,npart,nx,ny,nz,npart_new,seuil)
  ...
  call quick_sort(pdens,isort,npart)
  !write(*,*) "pdens", pdens(1)
  !write(*,*) "rhho", rho(isort(1))
  
  i=1
  !do while (pdens(i) < 1.d0/2.d0) !the 2 here can be increase up to delta or a fraction of delta I think RY
  !do while (pdens(i) < 1.d0/(seuil/2d0)) !RY. This is not enough. Should be seuil/6 at least.
  do while (pdens(i) < 1.d0/100.d0) !RY 10/22. This is tuned for 200c,nmin=100, lcdm, z=0. Should be a function of nmin and delta in principle roughly 100*(nmin/100)*(Delta/600)
  i=i+1  ! just select particle whose density is above a certain thresold value
  end do
  npart_new=i ! nb of particle in the working zone of the proc that fulfill the criterion
  if(procID==0)write(*,*)'npart_active=',npart_new,'/',npart
  !deallocate(rho)
  return
...
subroutine tri_pot(pdens,isort,npart,nx,ny,nz,npart_new,seuilpot)

  use mpi
  use modvar,only:potf
  implicit none
  integer::npart,nx,ny,nz,npart_new
  integer,dimension(1:npart)::isort
  real(8),dimension(1:npart)::pdens
  ...
  ! Tri des particules selon le potentiel
  !------------------------------------
  pdens=potf
  ...
  i=1
  !do while (pdens(i) < 1.d0/2.d0) !the 2 here can be increase up to delta or a fraction of delta I think RY
  do while (pdens(i) < seuilpot) !RY
     i=i+1
  end do
  npart_new=i ! same as before but if we choose to sort particles regarding of their potential
  if(procID==0)write(*,*)'npart_active=',npart_new,'/',npart

  return

end subroutine tri_pot
...
mtmp=0.d0 ! FT total mass in domain covered by proc
ntmp=0
...
if(periodic)then
   ...
   do i=1,npart
      ...
      !selection of particles of interest   !select particles in the working zone of the current process
      ...
      if(ok_part)then
         ntmp=ntmp+1 !nb of total part selected
         mtmp=mtmp+mpf(i) ! FT add mass
         ...
         ix=ddx  !index of the cell of the current part
         iy=ddy
         iz=ddz
         ...
         if(iyp1<0)iyp1=iyp1+ny !manage BC
         if(iy<0)iy=iy+ny
...
dx=1.d0/real(nx,kind=8) ! lenght of a cell in sim unit
dy=1.d0/real(ny,kind=8)
dz=1.d0/real(nz,kind=8)
...
subroutine spherover(nchar,npart,nx,ny,nz,&
                     ...
                     xxmin,xxmax,yymin,yymax,zzmin,zzmax,local_nx,local_ny,local_nz,periodic,level_max_global,xzmin_global&
                     ,xzmax_global) 
                     ! FT added level_max, xzmin, xzmax
!---------------------------------------------------------------------------
  use mpi
  use modvar
#ifdef READHDF5
  ...
  use modparameters
  Use modmpicom
  use modextract,only:potential,force
#endif  
  implicit none
  !integer::mpierr,procID,procNB
  ...
  integer, dimension(:), allocatable::npart_halo
  integer,dimension(:),allocatable::indx_ngp,isort_ngp
  integer(idprec),dimension(:),allocatable::idp
  integer(idprec),dimension(:),allocatable::idp_sel2
  integer,dimension(:),allocatable::first_ngp,num_ngp
  

  integer,dimension(:),allocatable::indx_sel,isort_sel,irank_sel
  real,dimension(:),allocatable::m_sel,pot_sel
  real,dimension(:,:),allocatable::x_sel,x_sel2,v_sel,for_sel
  real,dimension(:),allocatable::pot_sel2
  real(8),dimension(:),allocatable::distance
  real(8)::rayon_min,distance_new,factrmin

  integer::i,j,k,l,npart_sel,ipart,npart_sel2,npart_sel3,n_max,n_test
  ...
  logical::search_min_pot=.false.
  logical::search_max_npart_in_sphere=.true. !warning search_min_pot=.true. override this option
  logical::do_weight_self_binding_like=.true.
  ...
  Integer(idprec), dimension(:), allocatable :: halopartID !< Array containing the ID of the particles of a halo
  Real(kind=4), dimension(:), allocatable    :: halopartmass ! FT < Array containing the mass of the particles of a halo
  Real   (kind=4), dimension(:), allocatable ::halopartPot
  Integer(kind=4), dimension(:),allocatable :: haloMass !< Array containing the mass of the halos
  Real   (kind=4), dimension(:,:), allocatable :: halopartPos, halopartVel,halopartFor !< Array containing the position
  ...
  integer::local_nx,local_ny,local_nz          !input local utile grid size
  logical::periodic                            !input periodic boundary conditions
  real(8)::dx_smoothing_resolmax
  real(8)::abs_e_self_bind_rough,abs_e_self_bind_rough_max
  integer::iii,jjj,kkk
  integer :: level_max_global
  real(8) :: xzmin_global
  real(8) :: xzmax_global
  ...
  nmaxhalo=int(dble(npart)/mmin)  ! maximum number of halos possible
  allocate(idtab(1:nmaxhalo))
  allocate(masses(1:nmaxhalo))     !contains mass of each halo  
  allocate(npart_halo(1:nmaxhalo)) ! FT contains number of part of each halo
...
! factrmin = 1./16.d0  !RYtest
! rayon_min =0.62d0*scale/nx*factrmin !RYtest (4/3 pi)^(1/3)=0.62
  ! rayon_min = sqrt(3.d0)*scale/(2.d0*nx)
  if (do_weight_self_binding_like) then
     rayon_min = scale/nx
     dx_smoothing_resolmax=scale/nx/2.d0**6 !WARNING ROUGH (THIS TYPICAL SMOOTHING LENGTH CORRESPONDS TO 6 REFINEMENT LEVEL WHICH IS TYPICAL OF DMO SIM)
  else 
     rayon_min = sqrt(3.d0)*scale/(2.d0*nx)*factrmin ! ISC: improves HFM at small npart (12/07/2022)
  endif
  !filenames
  ...
  allocate(num_ngp  (1:local_nx*local_ny*local_nz))
  allocate(first_ngp(1:local_nx*local_ny*local_nz))
  allocate(indx_sel (Nsel))
  allocate(isort_sel(Nsel))
  allocate(distance (Nsel))
  allocate(x_sel    (Nsel,1:3))
  allocate(v_sel    (Nsel,1:3))
  if(potential)allocate(pot_sel    (Nsel))
  if(force)allocate(for_sel    (Nsel,1:3))
  allocate(x_sel2    (Nsel,1:3))
  if(potential)allocate(pot_sel2    (Nsel))
  allocate(m_sel    (Nsel))
  allocate(idp      (Nsel))
  allocate(idp_sel2      (Nsel))
  ncut2=2 !WARNING CHANGED FROM 1 TO 2 BY RY 02/12/23
  ncut3=1 !NEW RY 02/12/23
  
  ! Building particle linked list
  !--------------------------------------------
  if(procID==0)write(*,*)'Building particle linked list...'
  do i  = 1,npart
     i1 = floor(posf(1,i)-xxmin*scale)  ! index of particles
     i2 = floor(posf(2,i)-yymin*scale)
     i3 = floor(posf(3,i)-zzmin*scale)
     
     if(periodic) then  !take in account peridioc buffer
        if(i1<0)i1=i1+nx
        if(i1>=nx)i1=i1-nx
        if(i2<0)i2=i2+ny
        if(i2>=ny)i2=i2-ny
     ...
     indx_ngp(i) = 1+i1+i2*local_nx+i3*local_nx*local_ny !get 3D index of ngp of each particle
     
  end do
  if (procID==0)write(*,*) scale
  xsort=indx_ngp
  call quick_sort(xsort,isort_ngp,npart) !sort particles per increasing index --> isort_ngp array
  indx_ngp=xsort 
  ...
  first_ngp(:)=0 !contains the index of the 1st part of each group
  num_ngp(:)=0   !contains the number of part in each group --> group == ngp cell
  imin = 1
  do while(imin.le.npart)  !npart est npart local dans la zone du proc 
     ind  = indx_ngp(imin)
     i    = imin
     first_ngp(ind) = imin
  ...
  if(procID==0)write(*,*) 'ipart  halo#    npart   npart2   mass       x          y          z         it  radius     del   del2'
  ...
  !Loop on particles 
  do while( (ipart.le.npart_new).and.(dble(count_no_halo)/dble(npart_new).le.frac)) !stops when the fraction of particles that are
  !not in any halo drops under a fraction frac
  
  ! We choose most dense particles 
     !-------------------------------------------------
     if(xsort(isort(ipart)).gt.0..and.posf(1,isort(ipart))/scale.le.xzmax_global.and.&   !FT
             posf(1,isort(ipart))/scale.gt.xzmin_global.and.posf(2,isort(ipart))/scale.le.xzmax_global.and.&
             posf(2,isort(ipart))/scale.gt.xzmin_global.and.posf(3,isort(ipart))/scale.le.xzmax_global.and.&
             posf(3,isort(ipart))/scale.gt.xzmin_global)then !1st condition : eliminate particles which are already in a halo and 
             ! 2nd condition : only select the particles that are in the central zone of the sim i.e zoomed zone (FT) 
        !isort is an array containing the index of particle in posf in a deacreasing order of density
        ...
        !if(procID==0.and.ipart==261507)write(*,*)'Prosp. center:',ipart,iamas,xc,yc,zc  !RYtest
        xc0=xc; yc0=yc; zc0=zc !backup the center in case of iteration on the center (old versions)  !not used 
        ...
        ic = floor(xc-xxmin*scale) !extract index of the center of halo
        jc = floor(yc-yymin*scale)
        kc = floor(zc-zzmin*scale)
        if(periodic) then
           if(ic<0)ic=ic+nx
           if(ic>=nx)ic=ic-nx
           if(jc<0)jc=jc+ny
        ...
        indc=1+ic+jc*local_nx+kc*local_nx*local_ny !3D index of the center of halo
        mtotsel=0.d0
        do i=-ncut,ncut  ! triple loop that maps a cube [-ncut,+ncut] in the three directions centered on the halo center
           ii=i+ic
           if(ii.ge.nx)ii=ii-nx !manage BC dans les deux sens
           if(ii.lt.0) ii=ii+nx
           if (ii.ge.0.and.ii.lt.local_nx) then
              do j=-ncut,ncut
                 jj=j+jc
                 if(jj.ge.ny)jj=jj-ny
                 if(jj.lt.0) jj=jj+ny
                 if (jj.ge.0.and.jj.lt.local_ny) then
                    do k=-ncut,ncut
                       kk=k+kc
                       if(kk.ge.nz)kk=kk-nz
                       if(kk.lt.0) kk=kk+nz
                       
                       if (kk.ge.0.and.kk.lt.local_nz) then  !remainder: local_nz=2*(nxzone+2*nxbuffer)
                          
                          ind = 1+ii+jj*local_nx+kk*local_nx*local_ny !get index of the cell in the local grid
                          imin = first_ngp(ind) !index of first particle in the cell
                          imax = imin+num_ngp(ind)-1 !index of last particle in the cell
                          do ip = imin,imax ! loop over the particles in the cell
                             if(xsort(isort_ngp(ip)).gt.0.)then
                                is = is + 1
                                if(is > Nsel)then !Nsel maybe is the max number of part that can be selected
                                   write(*,*)procID,'WARNING: from ',nzonechar,&
                                           &' Increase Nsel in main program',' is=',is,' Nsel=',Nsel
                                   deallocate(structure,indx_ngp,isort_ngp)
                                   deallocate(masses,npart_halo,allpart,idtab,xctab,yctab,zctab)  
                                   deallocate(num_ngp,first_ngp,indx_sel)
                                   deallocate(isort_sel,distance,x_sel,v_sel)
                                   if(potential)deallocate(pot_sel)
                                   if(force)deallocate(for_sel)
                                   deallocate(x_sel2,m_sel,idp,idp_sel2)
                                     if(potential)deallocate(pot_sel2)
                                   return
                                end if
                                indx_sel(is) =  isort_ngp(ip)  !get id, pos, vel of particles selected
                                ...
                                if(potential)pot_sel(is)=potf(isort_ngp(ip))
                                if(force)for_sel(is,1)=forf(1,isort_ngp(ip))
                                if(force)for_sel(is,2)=forf(2,isort_ngp(ip))
                                if(force)for_sel(is,3)=forf(3,isort_ngp(ip))
                                m_sel(is) = mpf(isort_ngp(ip)) !get mass of particle selected
                                idp(is) = idf(isort_ngp(ip)) !index of the particule
                                mtotsel = mtotsel + m_sel(is)  ! total mass selected in the region around the center of mass
                             end if
                          end do
                       endif
                    end do
        ...
        npart_sel = is !number of particles selected
       
        !select particules that are in this cell plus ncut2 cell around for the barycentre loop 
        !as prospective center
        ! select a second set of particles in a cube -ncut2,+ncu2 in the three dims around the center cell
        is = 0
        do i=-ncut2,ncut2
           ii=i+ic
              ...
              do j=-ncut2,ncut2
                 ...
                 if (jj.ge.0.and.jj.lt.local_ny) then
                    do k=-ncut2,ncut2
                       ...
                       if(kk.ge.nz)kk=kk-nz
                       if(kk.lt.0) kk=kk+nz
                       if (kk.ge.0.and.kk.lt.local_nz) then
                          ind = 1+ii+jj*local_nx+kk*local_nx*local_ny
                          imin = first_ngp(ind)
                          ...
                          do ip = imin,imax
                             if(xsort(isort_ngp(ip)).gt.0.)then
                                ...
                                idp_sel2(is) = idf(isort_ngp(ip))
                                if(potential)pot_sel2(is) = potf(isort_ngp(ip))
                             end if
                          end do
                       endif
                    end do
        ...
        if(Mtotsel .ge. Mmin) then ! Mtotsel == mtotsel, fortran is case insensitive !!
           !loop on neighbouring particles to find the one with more neighbour
           !computed in the same way as one computes profile=> center is max density (or min potential)
           ...
           dc0 = dx*dx+dy*dy+dz*dz ! = 0.0 ?

           if(search_min_pot)then !RY 10/22 
              potmin=1d30 
              do j=1,npart_sel2
                 potnew=pot_sel2(j)
                 
                 if(potnew<potmin) then
                    potmin=potnew
                    xcnew=x_sel2(j,1)
                    ycnew=x_sel2(j,2)
                    zcnew=x_sel2(j,3)
              ...
              if(search_max_npart_in_sphere) then ! look for the maximum particles in sphere centered on particles that are
                      !neighbours of the first barycenter
                 
                 !NEW NCUT3>0 IMPLEMENTED: RY 04/12/23

                 if(ncut3>0) then 
                    dmax=1d30
                    n_max=0
                    abs_e_self_bind_rough_max=0.d0

                    
                    is = 0
                    do i=-ncut3,ncut3  !re select particle in range -ncut3 +ncut3 in the three dim around center cell
                       ii=i+ic
                       if(ii.ge.nx)ii=ii-nx
                       if(ii.lt.0) ii=ii+nx
                       if (ii.ge.0.and.ii.lt.local_nx) then
                                   ...
                                   if (kk.ge.0.and.kk.lt.local_nz) then
                                      ...
                                      do ip = imin,imax
                                         if(xsort(isort_ngp(ip)).gt.0.)then
                                            ...
                                            xcnew=posf(1,isort_ngp(ip))
                                            ycnew=posf(2,isort_ngp(ip))
                                            zcnew=posf(3,isort_ngp(ip))
                                            ...
                                            dmaxtmp=0.d0
                                            abs_e_self_bind_rough=0.d0
                                            iii=0
                                            do jjj=1,npart_sel2  ! sphere around all ncut3 particles in which the ncut2 particles
                                               xxnew=x_sel2(jjj,1) ! are listed
                                               yynew=x_sel2(jjj,2)
                                               zznew=x_sel2(jjj,3)
                                               ...
                                               if (distance_new.le.rayon_min**2) then
                                                  iii=iii+1 ! count particle is sphere of radius rayon_min
                                                  
                                                  if(do_weight_self_binding_like)abs_e_self_bind_rough=&
                                                          &abs_e_self_bind_rough+1.d0/sqrt(distance_new+dx_smoothing_resolmax**2)
                                                  
                                                  if(distance_new.ge.dmaxtmp**2)dmaxtmp=sqrt(distance_new)
                                               end if
                                            end do
                                            n_test=iii ! number of particles in sphere of radius rayon_min
                                            
                                            
                                            !update the barycentre if there are more particules in the sphere
                                            if(do_weight_self_binding_like)then
                                               if (abs_e_self_bind_rough.gt.abs_e_self_bind_rough_max)then
                                                  
                                                  xc=xcnew
                                                  yc=ycnew
                                                  zc=zcnew
                                                  iamas=idf(isort_ngp(ip))
                                                  abs_e_self_bind_rough_max=abs_e_self_bind_rough
                                                  n_max=n_test
                                                  dmax=dmaxtmp
                                               ...
                                               if (n_test.ge.n_max) then ! update the barycentre if there are more particules in the sphere
                                                  if(n_test.eq.n_max)then   !to get unique answer whatever ncpu
                                                    if (dmaxtmp.le.dmax) then ! if same number of part, get the sphere which is
                                                        n_max=n_test          ! the denser
                                                        xc=xcnew
                                                        yc=ycnew
                                                        zc=zcnew
                                                        iamas=idf(isort_ngp(ip))
                 ...
                 !---------------------------------------------------
                 !loop on all particules in the same cell as the current barycentre          !same routine than above but less
                 !precise, routine above loops over the 26 cells around, old version
                 !WARNING THIS IS NOT ACCURATE ONE SHOULD ALSO CONSIDER THE 26 CELLS AROUND
                 !WARNING NO XSORT
                 dmax=1d30
                 n_max=0
                 abs_e_self_bind_rough_max=0.d0
              ...
              endif !end search_max_npart_in_sphere
           endif
           ! Compute distances to the center
           !--------------------------------------------------
           
           do i = 1,npart_sel
              xx = x_sel(i,1) ! distance to all part in -ncut + ncut from the newly computed center
              yy = x_sel(i,2)
              zz = x_sel(i,3)
              ...
              distance(i) = dx*dx+dy*dy+dz*dz ! distance to all part in -ncut + ncut from the newly computed center
           end do
           ...
           !---------------------------------------------------
           i        = 1
           overdens = 2*seuil !to be sure to enter the while loop. total mass of 1 in total volume of 1 !
           amass    = m_sel(isort_sel(i)) ! mass of particles sorted by distance to barycenter m_sel(isort_sel(1)) is mass of
           !barycenter
           rayon    = 0.0d0      
           

           do while(i.lt.10.or.(overdens.gt.seuil.and.i.lt.npart_sel.and.rayon.lt.(real(ncut,kind=8)-sqrt(dc0))))
              i        = i+1  ! loop while overdensity has not reached the threshold or all particles in cnut zone have been treated
              rayon    = SQRT(distance(i)) ! or the distance to barycenter exceed ncut
              volume   = 2.d0/3.d0*twopi*rayon**3 ! volume inside the sphere
              amass    = amass+m_sel(isort_sel(i))  ! mass in the sphere
              overdens = amass/volume               !overdensity in the sphere
           ...
           if(overdens.gt.seuil)then ! overdensity seuil not reached within the selection radius (ncut), increase it !
              ...
              deallocate(distance,x_sel,v_sel,x_sel2,m_sel,idp,idp_sel2)
              if(potential)deallocate(pot_sel)
              if(potential)deallocate(pot_sel2)              
              if(force)deallocate(for_sel)
              return
              ...
              amass =amass-m_sel(isort_sel(i)) ! mass = last mass - last particle mass (otherwise above seuil overdensity)
              ...
              masse=i-1 !count nb of particle in the halo == mass if non zoom sim, otherwise problem ! real mass is amass
              if (procID==0) then
              end if
          end if
           ...
           if(masse.ge.Mmin.and.xc.le.xmaxzone.and.xc.gt.xminzone&  ! FT previous: masse replaced by amass
                .and.yc.le.ymaxzone.and.yc.gt.yminzone&
                .and.zc.le.zmaxzone.and.zc.gt.zminzone)then
              namas=namas+1 ! increase halo count
              !save id,npart,x,y,z for masst files
              idtab(namas)=iamas
              masses(namas)=amass  
              npart_halo(namas)=masse  ! FT 
              xctab(namas)=xc/scale
              yctab(namas)=yc/scale
              zctab(namas)=zc/scale
              
              gap=gap+masse !moove to next halo
              
              !write files
              
              if(procID==0)write(*,997)ipart,iamas,masse,masse2,dble(amass)/scale**3,xc/scale,yc/scale,zc/scale &
        ...
        !----------------------------------------------------
        if(masse.ge.Mmin.and.xc.le.xmaxzone.and.xc.gt.xminzone.and.& ! FT previous: masse replaced by amass
...
!  if(procID==0.and.ipart==261507)write(*,*)'Final center:',ipart,iamas,xc,yc,zc  !RYtest

!  if(ipart==261507)then  !RYtest
!     call MPI_BARRIER(MPI_COMM_WORLD,mpierr) !RYtest
!     call MPI_FINALIZE(mpierr) !RYtest
!     stop !RYtest
!  endif !RYtest
  ...
  if(write_masst_strct)then
     ...
     do i=1,namas 
        ...
        if(potential)write(26) (potf(allpart(j+gap-masses(i))),j=1,masses(i))
        if(force)write(26) ((forf(k,allpart(j+gap-masses(i))),k=1,3),j=1,masses(i))
     ...
     gap=0
     do i=1,namas      
        gap=gap+npart_halo(i) ! FT previous:  gap=gap+masses(i)
        haloMass(i)=npart_halo(i) ! FT previous: haloMass(i)=masses(i)
     ...
     allocate(halopartmass(gap)) ! FT allocate halopartmass
     allocate(halopartID(gap))
     if(potential)allocate(halopartPot (  gap))
     if(force)allocate(halopartFor (3,gap))
     gap=0
     do i=1,namas 
        gap = gap + npart_halo(i) ! FT previous: gap = gap + masses(i) 
        do j=1,npart_halo(i)      ! FT previous: do j=1,masses(i)
          halopartPos(:,j+gap-npart_halo(i))=posf(:,allpart(j+gap-npart_halo(i))) !FT previous: npart_halo(i) replaced by masses(i)
          halopartVel(:,j+gap-npart_halo(i))=velf(:,allpart(j+gap-npart_halo(i))) !in next 6 lines
          halopartID (  j+gap-npart_halo(i))=idf (  allpart(j+gap-npart_halo(i)))  
          halopartmass (  j+gap-npart_halo(i))=mpf (allpart(j+gap-npart_halo(i)))/real((2**level_max_global)**3,kind=8) ! convert mass back to 
          if(potential)halopartPot (  j+gap-npart_halo(i))=Potf (  allpart(j+gap-npart_halo(i)))                 ! sim units
          if(force)halopartFor(:,j+gap-npart_halo(i))=forf(:,allpart(j+gap-npart_halo(i))) 
       end do
     end do
    if(procID==0)then
        ...
        print*,'halopmass ',minval(halopartmass) ,maxval(halopartmass)
        if(potential)print*,'halopPot ',minval(halopartpot) ,maxval(halopartpot)
        if(force)print*,'halopFor ',minval(halopartfor) ,maxval(halopartfor)
     endif
     
     !WARNING: ISC 19/07/2022, FT 12/11/2024 --> write mass of particles + haloMass now contains the number of particles in halo not
                                                                                                          ! the mass of the halo !!!
     if(param%gatherwrite_factor <= 1) then
        if(potential)then
           if(force) then
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, &
                      &halopartVel, halopartID,halopartmass,halopartFor=halopartFor,halopartPot=halopartPot,inforamses=inforamses)
           else
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, &
                      &halopartVel, halopartID,halopartmass,halopartPot=halopartPot,inforamses=inforamses)
           endif
        else
           if(force) then
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, &
                      &halopartVel, halopartID,halopartmass,halopartFor=halopartFor,inforamses=inforamses)
           else
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, &
                      halopartVel, halopartID,halopartmass,inforamses=inforamses)
           endif
        endif
     else
        if(potential)then
            if(force) then
               call mpih5writehalopart(MpiSubCubeWrite,param, namas, gap, haloMass, haloID, halopartPos, &
                       halopartVel, halopartID,halopartmass,halopartFor=halopartFor,halopartPot=halopartPot,inforamses=inforamses)
            else
               call mpih5writehalopart(MpiSubCubeWrite,param, namas, gap, haloMass, haloID, halopartPos, &
                       halopartVel, halopartID,halopartmass,halopartPot=halopartPot,inforamses=inforamses)
     ...
     call  mpih5writehalomass(MPI_COMM_WORLD, param,haloNB_all, namas,nh, haloMass, halocomPos, &
     ...
     if(potential)deallocate(halopartPot)
     if(force)deallocate(halopartFor)
  ...
  if(IOGROUPSIZE>0) then
     ...
     call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
     if(procID==0)print*,'write Done'
  
  !deallocate
  deallocate(structure,indx_ngp,isort_ngp,num_ngp,first_ngp)
  deallocate(indx_sel,isort_sel,distance,x_sel,x_sel2,m_sel,idp,idp_sel2)
  deallocate(v_sel,allpart,masses,idtab,xctab,yctab,zctab)
  if(potential)deallocate(pot_sel)
  if(potential)deallocate(pot_sel2)
  if(force)deallocate(for_sel)
997 format (2I12,1x,I8,1x,I8,4(1x,1pe10.3),1x,I2,1x,6(E10.3,1x))
