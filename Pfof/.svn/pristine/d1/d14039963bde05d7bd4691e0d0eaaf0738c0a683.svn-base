...
Contains  
  ...
  Subroutine readramsesfiles()
    ...
    Call readinfoconegrav(filename, procID, infocone)
...
!       print *,'!!!!! NCELLPERLEVEL=',ncellperlevel
       ncelltab(ifile) = ncellloc
       ncellperleveltab(:,ifile) = ncellperlevel(:)
!       print *,'!!!!! NCELLTABPERLEVEL=',ncellperleveltab
    End Do
...
!    print *,'NCELLPERLEVEL=',ncellperlevel
  ...
  Subroutine h5writeconegrav()
    ...
    Integer(kind=hid_t) :: gr_id, grlvl_id, data_id, meta_id
    ...
    shellname = 'pfof_shell_cone_grav_data_'//trim(param%simulation_name)//'_'//charncoarse//'.h5'
    ...
    aname = 'file_type'
    adata = 'conegrav'
    Call hdf5_write_attr(meta_id, aname, adata)
    ...
    Call hdf5_write_mpi_data(meta_id,dsetname,ncube,nlevelperproc,ncellcube(:,beglvl:endlvl),&
    ...
    nctab(3) = ncz
    aname = 'ncube_array'
    Call hdf5_write_attr(meta_id, aname, 3, nctab)

    dimtab(1) = param%cone_max_radius
    dimtab(2) = hy
    dimtab(3) = hz
    aname = 'dimensions_array'
    Call hdf5_write_attr(meta_id, aname, 3, dimtab)

    Call hdf5_close_group(meta_id)


    groupname='data'
    Call hdf5_create_group(file_id, groupname, data_id)
    ...
    Do ilvl = 1, param%nlevel
       ...
       Call hdf5_create_group(data_id, groupname, grlvl_id)
...
!!$             End If
             ...
             dsetname = 'gravitational_field_cell'
