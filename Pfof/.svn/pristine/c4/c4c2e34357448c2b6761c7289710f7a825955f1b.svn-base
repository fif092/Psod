...
Contains
  ...
  Subroutine extract_cuboid()
    ...
    Type(Type_parameter_pfof_snap) :: pfof_parameter
    ...
    Call h5read_meta_pfof_parameter(file_id, pfof_parameter)
...
!!$    Call hdf5_read_attr(gr_id,aname,potentialattr)
    If(potential .And. .not.pfof_parameter%do_read_potential) Then
...
!!$    Call hdf5_read_attr(gr_id,aname,forceattr)
    If(force .And. .not.pfof_parameter%do_read_gravitational_field) Then
       Print *,'Gravitational field requested but not found in hdf5 file. Exit.'
       Stop 10
    End If
    ...
    Do ifile=1,nfile-1
       ...
       If(fileexist) Then
          ...
          Call hdf5_open_group(file_id, grname, gr_id)
          aname = 'boundaries'
          Call hdf5_read_attr(gr_id,aname,6,boundaries(:,ifile+1))

          dname = 'npart_grp_array'
          Call hdf5_read_data(gr_id,dname,ngroup,npartpergroup(:,ifile+1))
          Call hdf5_close_group(gr_id)
          Call hdf5_close_file(file_id)
       End If
    End Do
