module modvar
  ...
  integer(idprec),dimension(:),allocatable::idp8 !id
  real,dimension(:,:),allocatable::x,vel   !position velocity
...
program sphericaloverdensity
  ...
  integer(idprecread),dimension(:), allocatable :: id
  ...
  integer(8)::kk
  character::idpchar*5,filename*250
  ...
  do while(found.eqv..true.)
     ...
     filename=trim(root)//'_cube_'//idpchar
...
! Loop on the utile zones (cube of number nzone) that will be analyzed 
  do nzone=firstp,lastp
     ...
     countinfile: DO kk=0,26
        ...
        filename=trim(root)//'_cube_'//idpchar        
        open(uinout,file=filename, status='Old', form='Unformatted')
        read(uinout) nloc
        read(uinout) proc_ID
     ...
     allocate(idp8(1:npart),mp(1:npart))
     allocate(xsort(1:npart),isort(1:npart))
     
     !fill final arrays x, vel, idp8, mp
     ifillpart=0
     loadfile: DO kk=0,26
        !read
        write(idpchar,'(I5.5)') list_cpu(kk)
     ...
     if(procID==0)then
        ...
        print*,'Proc',procid,'minmax(x)',minval(x(:,1)),maxval(x(:,1))
        print*,'Proc',procid,'ymin ymax',ymin,ymax
        print*,'Proc',procid,'minmax(y)',minval(x(:,2)),maxval(x(:,2))
        print*,'Proc',procid,'zmin zmax',zmin,zmax
        print*,'Proc',procid,'minmax(z)',minval(x(:,3)),maxval(x(:,3))
     endif
     ...
     deallocate(x,vel,idp8,mp,xsort,isort)
     call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
...
!---------------------------------------------------------------------------
  ...
  ! allpart contains the index of the parts of each halo (in x, vel and idp8 arrays) 
              ...
              do k=-ncut,ncut
                 ...
                 do ip = imin,imax
                    if(xsort(isort_ngp(ip)).gt.0.)then
                       ...
                       idp(is) = idp8(isort_ngp(ip)) !index of the particule
  ...
  if(write_masst_strct)then
     ...
     x=x/scale !conversion forward to be improved
     gap=0
     ...
     do i=1,namas 
        ...
        write(26) (idp8(allpart(j+gap-masses(i))),j=1,masses(i))
     enddo
     close(26)
     x=x*scale !conversion backward to be improved
  endif
