...
!!!namas = id(1)
    ...
    particules : Do i=1, mynpart
       ...
       If (pos(1,ipart) == 1.0) pos(1,ipart) = 0.00000
       If (pos(2,ipart) == 1.0) pos(2,ipart) = 0.00000
       If (pos(3,ipart) == 1.0) pos(3,ipart) = 0.00000
    ...
    If(dotimings) Then
       ...
       If(procID==0) Print *,'Process 0 spent ', tSelectHalo, ' s selecting haloes with Mass > ',Mmin
       timeInt = Mpi_Wtime()
    End If
    ...
    If(potential .and. force) Then
       Call h5writehalopart(MpiCube, haloNB, halopartNB, haloMass, haloID, halopartPos, halopartVel,&
            halopartID, halopartFor=halopartFor, halopartPot=halopartPot)
    Else If(potential .and. .not. force ) Then
       Call h5writehalopart(MpiCube, haloNB, halopartNB, haloMass, haloID, halopartPos, halopartVel,&
            halopartID, halopartPot=halopartPot)
    Else If(force .and. .not. potential) Then
       Call h5writehalopart(MpiCube, haloNB, halopartNB, haloMass, haloID, halopartPos, halopartVel,&
            halopartID, halopartFor=halopartFor)
    Else
       Call h5writehalopart(MpiCube, haloNB, halopartNB, haloMass, haloID, halopartPos, halopartVel,&
            halopartID)
    End If
    ...
    Call mpih5writehalomass(MpiCube,haloNB_all, haloNB, nh, haloMass, halocomPos, halocomVel, haloID, &
         haloRadius, haloSubHaloNB)
    ...
    If(procID==0) Then
       ...
       Print *,'******************************************************'
       Print *,'End of pFoF!'
       Print *,'Number of haloes with mass > ', Mmin,':',haloNB_all
       Print *,'******************************************************'
       Print *,' '

       Write(Ulog,*) 'Number of haloes with mass > ', Mmin,':',haloNB_all
