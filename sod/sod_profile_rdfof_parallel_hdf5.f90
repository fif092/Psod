...
!                 if(procID==0.and.ipart==261507)write(555,*)j,x_sel2(j,1),x_sel2(j,2),x_sel2(j,3),pot_sel2(j),idp_sel2(j) !RYtest
           ...
           do i = 1,npart_sel
              ...
              !if (procID==0)write(*,*) ""
              !if (procID==0)write(*,*) "xx", xx
              !if (procID==0)write(*,*) "yy", yy
              !if (procID==0)write(*,*) "zz", zz
              !if (procID==0)write(*,*) ""
             ...
             ! if (procID==0)write(*,*) ""
             ! if (procID==0)write(*,*) "distance", distance(i)
             ! if (procID==0)write(*,*) ""
           ...
           do while(i.lt.10.or.(overdens.gt.seuil.and.i.lt.npart_sel.and.rayon.lt.(real(ncut,kind=8)-sqrt(dc0))))
              ...
              if (procID==0)write(*,*) ""
              if (procID==0)write(*,*) "rayon", rayon
              if (procID==0)write(*,*) "amass1", amass
              if (procID==0)write(*,*) "volume", volume
              if (procID==0)write(*,*) "overdens", overdens
              if (procID==0)write(*,*) ""
        ...
        if(potential)then
           if(force) then
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, &
                      &halopartVel, halopartID,halopartFor=halopartFor,halopartPot=halopartPot,inforamses=inforamses)
           else
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, &
