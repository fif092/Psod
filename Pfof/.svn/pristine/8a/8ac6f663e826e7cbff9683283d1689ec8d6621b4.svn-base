...
Contains
  ...
  Subroutine gatherhaloes(mpicomm)
    ...
    procMasse : Do iproc = 1,procNB-1
       ...
       If(nbsend /= 0) Then
          ...
          If(potential) Allocate(potSend(nbsend))
          If(force) Allocate(forSend(3,nbsend))
          ...
          Do i=1, mynpart
             If(structure(i)>= smin .and. structure(i)<= smax) Then
                ...
                posSend(:,ind) = pos(:,i)
                velSend(:,ind) = vel(:,i)
                If(potential) potSend(ind) = pot(i)
                If(force) forSend(:,ind) = for(:,i)
                idSend(ind) = id(i)
                ind = ind+1
             End If
          End Do
          ...
          If(potential) Call Mpi_Isend(potSend, nbsend, Mpi_Real, sendID, 5,mpicomm,mpireqs5,mpierr)
          If(force) Call Mpi_Isend(forSend,3*nbsend,Mpi_Real,sendID,6,mpicomm,mpireqs6,mpierr)
       End If
       ...
       If(nbrec /= 0) Then
          ...
          If(potential) Call Mpi_IRecv(potf(recvpoint), nbrec,  Mpi_Real, recvID,5,mpicomm,mpireqr5,mpierr)
          If(force) Call Mpi_IRecv(forf(1,recvpoint),3*nbrec,Mpi_Real,recvID,6,mpicomm,mpireqr6,mpierr)
       ...
       If(nbsend/=0) Then
          ...
          If(potential) Call Mpi_Wait(mpireqs5,mpistat,mpierr)
          If(force) Call Mpi_Wait(mpireqs6,mpistat,mpierr)
          Deallocate(strSend,posSend,idSend,velSend)
          If(potential) Deallocate(potSend)
          If(force) Deallocate(forSend)
       End If
       If(nbrec/=0) Then
          Call Mpi_Wait(mpireqr1,mpistat,mpierr)
          ...
          If(potential) Call Mpi_Wait(mpireqr5,mpistat,mpierr)
          If(force) Call Mpi_Wait(mpireqr6,mpistat,mpierr)
       End If
    ...
    Do i=1, mynpart
       If(structure(i)>= strNBbeg .and. structure(i)<= strNBend) Then
          ...
          idf(recvpoint)  = id(i)
          If(potential) potf(recvpoint) = pot(i)
          If(force) forf(:,recvpoint) = for(:,i)
  ...
  Subroutine selecthaloes()
    ...
    Use modvariables
    Use modparameters, only : Mmin, potential, force
    ...
    If(potential) Allocate(halopartPot(halopartNB))
    If(force) Allocate(halopartFor(3,halopartNB))
    ...
    Do i = 1, nbhid
       If(halomasstmp(i) >= Mmin) Then
          ...
          If(potential) halopartPot(fp:lp) = potf(fi:li)
          If(force) halopartFor(:,fp:lp) = forf(:,fi:li)
