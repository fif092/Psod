...
Contains
  ...
  Subroutine extract_cuboid()
    ...
    Allocate(mass(npart2extract)) !ftor
          ...
          If(fileexist) Then
             ...
             Do igroup=1,ngroupperfile(ifile)
                ...
                If( npart/=0 ) Then ! there is at least 1 part. to read in this group
                   ...
                   dname='mass_part'  !ftor
                   Call hdf5_read_data(gr_id, dname, npart, mass(indpart:indpart+npart-1)) !ftor
