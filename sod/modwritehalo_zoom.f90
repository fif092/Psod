...
Contains
  ...
  Subroutine h5writehalopart(param_pfof, haloNB, halopartNB, haloMass, haloID, &
       halopartPos, halopartVel, halopartID, halopartMass,halopartFor, halopartPot, halopartRamsesID,&
       inforamses, infocone) ! FT add halopartMass
    ...
    Real(kind=4), dimension(halopartNB), intent(in), target :: halopartMass !< Array containing the ID of the
    !< particles belonging to the halos
    ...
    If(haloNB/=0) Then
       ...
       Do ih = 1, haloNB
          ...
          dsetname = 'mass_part'           ! FT
          Call hdf5_write_data(gr_halo_id, dsetname, haloMass(ih), &  ! FT
               halopartMass(hptr:hptr+haloMass(ih)-1))         ! FT
  ...
  Subroutine mpih5writehalopart(mpicomm, param_pfof, haloNB, halopartNB, haloMass, haloID, &
    ...
    Integer(kind=4), intent(in) :: halopartNB  !< Number of particles belonging to the halos
    Integer(kind=PRI), dimension(haloNB), intent(in) :: haloID !< Array containing the ID of the halos
    Real(kind=4), dimension(halopartNB), intent(in), target :: halopartMass !< FT Array containing the masses of the
    !< particles belonging to the halos
    Integer(kind=PRI), dimension(halopartNB), intent(in), target :: halopartID !< Array containing the ID of
    !particles belonging to the halos 
    Integer(kind=4), dimension(haloNB), intent(in) :: haloMass !< Array containing the mass of the halos
    Real   (kind=4), dimension(3,halopartNB), intent(in), target :: halopartPos, halopartVel !< Array containing the position
    !< and velocity of the particles belonging to the halos
...
!int(halopartNB,kind=8)
    ...
    If(haloNBall/=0) Then
       ...
       Do ih = 1, haloNBall
          ...
          dsetname = 'mass_part'  ! FT  write masses of halo particles
          Call hdf5_write_mpi_data(gr_halo_id, dsetname, partnb, &  ! FT
               halopartMass(fp:lp), mpicomm, empty)                 ! FT
  ...
  !=======================================================================
  !> This subroutine writes, for each halo, its number of particles,  
  ...
  Subroutine mpih5writehalomass(mpicomm, param_pfof, haloNB_all, haloNB, nh, &
       haloMass, halocomPos, halocomVel, haloID, haloRadius, haloSubHaloNB, inforamses, infocone) 
       ! haloMass in fact contains the number of part in each halo, not its mass !!
    ...
    Class Is(Type_parameter_pfof_cone)
       ...
       islast = .true.
    Class is (Type_parameter_psod_snap)
       filename = 'psod_halo_snap_part_hfprop_'//trim(param_pfof%simulation_name)//'.h5'
       codename='psod_snap'
       halofinder = 'psod'
       islast = .false.
    End Select
