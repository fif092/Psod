...
Contains
  ...
  Subroutine parafof()
    ...
    Real(kind=SP), dimension(:,:), allocatable :: halopartPos, halopartVel
    Real(kind=DP), dimension(:,:), allocatable :: halocomPos, halocomVel
    Real(kind=SP), dimension(:,:), allocatable :: posAva,posArr,posBas,posDro,posGau,posHau
    Real(kind=DP), dimension(3) :: delta
    Real(kind=SP) :: r, d, size, size_fof, xx, yy, zz
...
!    if(procID==procNB-1)strNBend=nptot!!!!!Y   FR : Pas necessaire !
...
!          if(sendID==procNB-1)smax=nptot!!!!Y
  ...
  Subroutine init(n,np,ngpp,x,pile,adfirst,npcase,xmin,ymin,zmin,t)
    ...
    Real(kind=SP), Intent(in) :: t                      !< resolution of the cosmological simulation times refine_fof factor
  ...
  Subroutine findbridge(nloc, nrecv, pos1, pos2, nbridge1, bridge1, nbridge2, bridge2, r2, v1, v2)
    ...
    Real(kind=SP), intent(in), dimension(3,*) :: pos1   !< positions of local particles flagged for link detection in direction 1
    Real(kind=SP), intent(in), dimension(3,*) :: pos2   !< positions of distant particles flagged for link detection in direction 2
    Real(kind=SP), intent(in) :: r2                     !< percolation length
    ...
    Real(kind=SP) :: d, dx, dy, dz
    Real(kind=SP), dimension(:,:), allocatable :: posRec
