...
program sphericaloverdensity
  ...
  ! Count the total number of particles in the simulation  ->SHOULD BE INTEGER8 NO???
  nthalo=0
  if(procID==0)then
     do kk=0,ncpu-1
...
contains
  subroutine read_params
    ...
    use modparameters
      ...
      param%grpsize=0
      param%gatherread_factor=1
      param%snapshot=nsnap         
      param%gatherwrite_factor=1
...
!---------------------------------------------------------------------------
  ...
  if(procID==0)print*,'nmaxhalo=',nmaxhalo
...
call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
  if(procID==0)print*,'nx*ny*nz=',nx*ny*nz
  if(procID==0)print*,'Nsel=',nsel
