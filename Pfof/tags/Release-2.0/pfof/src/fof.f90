...
Program friend
  ...
  ! 3*bit_size(Mmin)/8 : 3x Integer(kind=4)   grpsize, Mmin, Mmax
  ! kind(perco)        : 1x Real(kind=4)      perco
  ! 6*4                : 6x Logical           star, metal, outcube, dofof, readfromcube, dotimings

  h_length = 80*4 + 3 + 3* bit_size(Mmin)/8 + kind(perco) + 6*4
  ...
  If(procID==0) Then
     ...
     Read(10,'(A80)') root
     Read(10,*) code_index
     Read(10,'(A80)') pathinput
     Read(10,'(A80)') namepart
     Read(10,'(A80)') nameinfo
     Read(10,*) grpsize
     Read(10,*) perco
     Read(10,*) Mmin
     Read(10,*) Mmax
     ...
     Call Mpi_Pack(        root, 80, Mpi_Character, header, h_length, h_pos, Mpi_Comm_World, mpierr)
     Call Mpi_Pack(  code_index,  3, Mpi_Character, header, h_length, h_pos, Mpi_Comm_World, mpierr)
     Call Mpi_Pack(   pathinput, 80, Mpi_Character, header, h_length, h_pos, Mpi_Comm_World, mpierr)
     Call Mpi_Pack(    namepart, 80, Mpi_Character, header, h_length, h_pos, Mpi_Comm_World, mpierr)
     Call Mpi_Pack(    nameinfo, 80, Mpi_Character, header, h_length, h_pos, Mpi_Comm_World, mpierr)
  ...
  If(procID /= 0) Then
     Call Mpi_Unpack(header, h_length, h_pos,         root, 80, Mpi_Character, Mpi_Comm_World, mpierr)
     Call Mpi_Unpack(header, h_length, h_pos,   code_index,  3, Mpi_Character, Mpi_Comm_World, mpierr)
     Call Mpi_Unpack(header, h_length, h_pos,    pathinput, 80, Mpi_Character, Mpi_Comm_World, mpierr)
     Call Mpi_Unpack(header, h_length, h_pos,     namepart, 80, Mpi_Character, Mpi_Comm_World, mpierr)
     Call Mpi_Unpack(header, h_length, h_pos,     nameinfo, 80, Mpi_Character, Mpi_Comm_World, mpierr)
  ...
  If (code_index.eq.'RA2' .or. code_index.eq.'RA3') Then
     Call RAMSES_lecture()
  Else
