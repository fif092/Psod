...
Contains
  ...
  Subroutine h5writehalopart(outroot,mpicomm, haloNB, halopartNB, haloMass, haloID, halopartPos, halopartVel, &
       halopartID,halopartFor, halopartPot)
    !add outroot ->modif ry
    Use modconstant,only:Mpi_PRI  !!!!modif RY
    Use modhdf5
    Use modparameters
    Use modmpicom
    Use modvariables
!    Use modxdmf
    Implicit none
    Character(len=400) :: outroot !< root of output !!!modif ry
    ...
    filestrct = trim(outroot)//'_halo_'//pid_char//'.h5'
    filebase = trim(outroot)//'_halo'
...
!!$    End If
  ...
  Subroutine mpih5writehalomass(outroot,mpicomm, haloNB_all, haloNB,nh, haloMass, halocomPos, halocomVel, &
...
!!$    Integer(kind=4), dimension(haloNB), intent(in), target :: haloSubHaloNB !< Number of subhalos in each halo
    ...
    filename = trim(outroot)//'_halomass.h5'
