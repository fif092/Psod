...
! Loop on the utile zones (cube of number nzone) that will be analyzed 
  do nzone=firstp,lastp   !ftor nzone is the 1D index of the zone that is currently analyzed by the process
     ...
     !Boundary conditions: these quantities can be below 0 or above 1 !ftor ?
     ...
     !if(procID==0)write(*,*) 'rxsmall', rxsmall, 'rysmall', rysmall, 'rzsmall', rzsmall !ftor
...
!           endif
        ...
        do i=1,npartini  ! second loop to get pos,vel, id and mass of particles in the working region
           ...
           if ((dx.lt.rxsmall.or.dx.gt.rxlarge).and.&
              ...
              do j=1,3
                 ...
                 if(force)forf(j,ifillpart)=for(j,i)
              enddo
              mpf(ifillpart)=mass(i)    ! ftor okay with zoom sims get mass of each particle 
              !write(*,*) "particle mass =", mpf(ifillpart) !ftor
              idf(ifillpart)=id(i)
              if(potential)potf(ifillpart)=pot(i)
           endif           
        end do
     ...
     if (docic)then
        !mpf=mpf /real(nparttotal,kind=8) !conversion forward could be improved !ftor (line commented)
     ...
     Mmin=(1.0d0/((2**level_max)**3))*real(nmin,kind=8)*real(nparttotal,kind=8)
     write(*,*) "Mmin", Mmin
     ...
     if(dopot)then
        ...
        if(docic)then
           ...
           !if (procID == 0) then !! ok, densest part are in zoomed zone --> cic seems to be ok !
           !do i =1,1000
           !   write(*,*) "mass max", mpf(isort(i))/real(nparttotal,kind=8)
           !end do
      ! end if   
       else
     ...
     call spherover(nchar,npart,nx,ny,nz,&
          ...
          xxmin,xxmax,yymin,yymax,zzmin,zzmax,local_nx,local_ny,local_nz,periodic)
  ...
  subroutine read_params
    ...
    use modparameters
      ...
      if (n < 7) then
         ...
         print *, '            [-level_max] (for zoom simulations: level of heaviest particles in the sim)'  !ftor
         print *, '            [-hlp]          (optional)'
         endif
         call MPI_FINALIZE(mpierr)
         stop
...
if(periodic)then
   ...
   do i=1,npart
      ...
      if(ok_part)then
         ...
         mtmp=mtmp+mpf(i) !total mass selected
...
!---------------------------------------------------------------------------
  ...
  Real(kind=4), dimension(:), allocatable    :: halopartmass ! ftor < Array containing the mass of the particles of a halo
...
! rayon_min =0.62d0*scale/nx*factrmin !RYtest (4/3 pi)^(1/3)=0.62
  ...
  ipart=1
  !Loop on particles   !npart_new is probably a redefinition of npart
  ...
  !not in any halo drops under a fraction frac
     ...
     !-------------------------------------------------
     if(xsort(isort(ipart)).gt.0..and.posf(1,isort(ipart))/scale.le.0.6.and.&
             posf(1,isort(ipart))/scale.gt.0.4.and.posf(2,isort(ipart))/scale.le.0.6.and.&
             posf(2,isort(ipart))/scale.gt.0.4.and.posf(3,isort(ipart))/scale.le.0.6.and.&
             posf(3,isort(ipart))/scale.gt.0.4)then !this is to eliminate particles which are already in a halo and 
             ! only select the particles that are in the central zone of the sim i.e zoomed zone (ftor)
                       ...
                       if (kk.ge.0.and.kk.lt.local_nz) then  !remainder: local_nz=2*(nxzone+2*nxbuffer)
                          ...
                          do ip = imin,imax ! loop over the particles in the cell
                             if(xsort(isort_ngp(ip)).gt.0.)then
                                ...
                                !write(*,*) "mpf here" , mpf(isort_ngp(ip))  !ftor
...
!                 if(procID==0.and.ipart==261507)write(555,*)j,x_sel2(j,1),x_sel2(j,2),x_sel2(j,3),pot_sel2(j),idp_sel2(j) !RYtest
           ...
           !---------------------------------------------------
           i        = 1
           overdens = 4*seuil !2*seuil because grid is 2*thiner than scale. seuil in unit of mean density --> total mass of 1 in total volume of 1 !
           amass    = m_sel(isort_sel(i)) ! mass of particles sorted by distance to barycenter m_sel(isort_sel(1)) is mass
           !if (procID==0) then
           !   write(*,*) "amass", amass/real(nparttotal,kind=8)
           !end if
           rayon    = 0.0d0               ! of particle that lies at the barycener
...
!           write(*,*) "halo mass", amass
        ...
        do i = 1,masse
           if(amass.ge.Mmin.and.xc.le.xmaxzone.and.xc.gt.xminzone&
              ...
              !if(procID==0.and.ipart==261507)write(444,*)posf(1,indx_sel(isort_sel(i))),posf(2,indx_sel(isort_sel(i))),posf(3,indx_sel(isort_sel(i))),potf(indx_sel(isort_sel(i))) !RYtest
  ...
  if(write_masst_strct)then
     ...
     do i=1,namas 
        ...
        do j=1,masses(i)
           ...
           halopartmass (  j+gap-masses(i))=mpf (  allpart(j+gap-masses(i)))/real(nparttotal,kind=8)  ! ftor
           if(potential)halopartPot (  j+gap-masses(i))=Potf (  allpart(j+gap-masses(i)))
           if(force)halopartFor(:,j+gap-masses(i))=forf(:,allpart(j+gap-masses(i)))
        end do
     end do
    if(procID==0)then
     ...
     !WARNING: ISC 19/07/2022, FT 12/11/2024 --> write mass of particles
