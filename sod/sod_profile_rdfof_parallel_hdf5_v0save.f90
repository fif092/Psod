...
#ifndef READHDF5
  integer(idprec),dimension(:),allocatable::id !id
  real,dimension(:,:),allocatable::pos,vel   !position velocity
#endif
  real,dimension(:),allocatable::mp   !mass
  real(8),dimension(:),allocatable::xsort !density
  integer,dimension(:),allocatable::isort !sort index
  integer::IOGROUPSIZE,dummy_io,tag=2700,info  !IO tickets parameters
...
program sphericaloverdensity
  ...
  !+ test version ultra original sod + test profile + selection moins large rayon_min + test occupation memoire etc...
  !---------------------------------------------------------------------
...
! Loop on the utile zones (cube of number nzone) that will be analyzed 
  do nzone=firstp,lastp
     ...
     if(IOGROUPSIZE>0) then
        ...
        allocate(pos(1:npart,1:ndim),vel(1:npart,1:ndim))
        allocate(id(1:npart))
        allocate(mp(1:npart))
        ...
        loadfile: DO kk=0,26
           ...
           do i=1,nloc
              ...
              if ((dx.lt.rxsmall.or.dx.gt.rxlarge).and.&
                 ...
                 do j=1,3
                    pos(ifillpart,j)=cdm(j,i)
                    vel(ifillpart,j)=v  (j,i)
                 enddo
                 mp      (ifillpart)=1.  !non zoom sims
                 id(ifillpart)=idtmp(i)
        ...
        type = 'cuboid'
        filename='/data/yrasera/prepa4096/boxlen80_n128_lcdmw7/post/fof/output_00004/fof_boxlen80_n128_lcdmw7_mpisortedcube'
        center(1)=xc
        center(2)=yc
        center(3)=zc
        ...
        print*,'procID there 0',procID
        call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
     
        Call extract_cuboid()
        print*,'procID there 1',procID
        call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
        ...
        if(procID==0)print*,'Proc',procID,' ifillpart',ifillpart,' npart=',npart
     ...
     if(procID==0)then
        ...
        print*,'Proc',procid,'minmax(x)',minval(pos(:,1)),maxval(pos(:,1))
        print*,'Proc',procid,'ymin ymax',ymin,ymax
        print*,'Proc',procid,'minmax(y)',minval(pos(:,2)),maxval(pos(:,2))
        print*,'Proc',procid,'zmin zmax',zmin,zmax
        print*,'Proc',procid,'minmax(z)',minval(pos(:,3)),maxval(pos(:,3))
     endif
     ...
     if (docic)then
        mp=mp/real(nparttotal,kind=8) !conversion forward could be improved
        ...
        mp=mp*real(nparttotal,kind=8) !conversion backward could be improved
     endif
...
subroutine tri_ngp(pdens,isort,npart,nx,ny,nz,npart_new)
#ifndef READHDF5
  use modvar,only:pos,mp
#else
  use modvariables,only:pos
  use modvar,only:mp
#endif
  use mpi
  implicit none
  ...
  !-----------------------------------
  do i = 1,npart
     i1 = int(pos(i,1)*dble(ngx)/dble(nx))
     i2 = int(pos(i,2)*dble(ngy)/dble(ny))
     i3 = int(pos(i,3)*dble(ngz)/dble(nz))
...
if(periodic)then
   ...
   do i=1,npart
      if (pos(i,1)==1.)write(*,*)'WARNING x=1'
      if (pos(i,2)==1.)write(*,*)'WARNING y=1'
      if (pos(i,3)==1.)write(*,*)'WARNING z=1'
      !selection of particles of interest
      ok_part=(((pos(i,1)>=xmin   .and.pos(i,1)<=xmax   ).or.     &
           &    (pos(i,1)>=xmin+1.d0.and.pos(i,1)<=xmax+1.d0).or.     &
           &    (pos(i,1)>=xmin-1.d0.and.pos(i,1)<=xmax-1.d0))   .and.&
           &   ((pos(i,2)>=ymin   .and.pos(i,2)<=ymax   ).or.     &
           &    (pos(i,2)>=ymin+1.d0.and.pos(i,2)<=ymax+1.d0).or.     &
           &    (pos(i,2)>=ymin-1.d0.and.pos(i,2)<=ymax-1.d0))   .and.&
           &   ((pos(i,3)>=zmin   .and.pos(i,3)<=zmax   ).or.     &
           &    (pos(i,3)>=zmin+1.d0.and.pos(i,3)<=zmax+1.d0).or.     &
           &    (pos(i,3)>=zmin-1.d0.and.pos(i,3)<=zmax-1.d0)))
      
      if(ok_part)then
         ntmp=ntmp+1
         mtmp=mtmp+mp(i)
         !compute distances from edges of cells in all direction
         ddx=(pos(i,idim)-xxmin)/dx
         ddy=(pos(i,jdim)-yymin)/dy
         ddz=(pos(i,kdim)-zzmin)/dz
         ix=ddx
         iy=ddy
         iz=ddz
   ...
   do i=1,npart
      ok_part=(pos(i,1)>=xmin.and.pos(i,1)<xmax.and. &
           &   pos(i,2)>=ymin.and.pos(i,2)<ymax.and. &
           &   pos(i,3)>=zmin.and.pos(i,3)<zmax)
      if(ok_part)then
...
use mpi
#ifndef READHDF5
  use modvar,only:pos                        !input local array x  (positions between [0.,1.[)
#else
  use modvariables,only:pos
#endif
...
do i=1,npart
   if(periodic) then
      ok_part=(((pos(i,1)>=xxmin   .and.pos(i,1)<=xxmax   ).or.     &
           &    (pos(i,1)>=xxmin+1.d0.and.pos(i,1)<=xxmax+1.d0).or.     &
           &    (pos(i,1)>=xxmin-1.d0.and.pos(i,1)<=xxmax-1.d0))   .and.&
           &   ((pos(i,2)>=yymin   .and.pos(i,2)<=yymax   ).or.     &
           &    (pos(i,2)>=yymin+1.d0.and.pos(i,2)<=yymax+1.d0).or.     &
           &    (pos(i,2)>=yymin-1.d0.and.pos(i,2)<=yymax-1.d0))   .and.&
           &   ((pos(i,3)>=zzmin   .and.pos(i,3)<=zzmax   ).or.     &
           &    (pos(i,3)>=zzmin+1.d0.and.pos(i,3)<=zzmax+1.d0).or.     &
           &    (pos(i,3)>=zzmin-1.d0.and.pos(i,3)<=zzmax-1.d0)))
   else
      ok_part=(pos(i,1)>=xxmin.and.pos(i,1)<xxmax.and. &
           &   pos(i,2)>=yymin.and.pos(i,2)<yymax.and. &
           &   pos(i,3)>=zzmin.and.pos(i,3)<zzmax)
   endif
...
!---------------------------------------------------------------------------
  ...
  do while( (ipart.le.npart_new).and.(dble(count_no_halo)/dble(npart_new).le.frac))
     ...
     if(xsort(isort(ipart)).gt.0.)then !this is to eliminate particles which are already in a halo
        ...
        xc = pos(isort(ipart),1)
        yc = pos(isort(ipart),2)
        zc = pos(isort(ipart),3)

        ! the halo identifier is the id of the barycentre
        iamas = id(isort(ipart))
              ...
              do k=-ncut,ncut
                 ...
                 do ip = imin,imax
                    if(xsort(isort_ngp(ip)).gt.0.)then
                       ...
                       x_sel(is,1) = pos(isort_ngp(ip),1)
                       x_sel(is,2) = pos(isort_ngp(ip),2)
                       x_sel(is,3) = pos(isort_ngp(ip),3)
                       v_sel(is,1) = vel(isort_ngp(ip),1)
                       v_sel(is,2) = vel(isort_ngp(ip),2)
                       v_sel(is,3) = vel(isort_ngp(ip),3)
                       m_sel(is) = mp (isort_ngp(ip))
                       idp(is) = id(isort_ngp(ip)) !index of the particule
           ...
           do j=-ncut2,ncut2
              ...
              do k=-ncut2,ncut2
                 ...
                 do ip = imin,imax
                    if(xsort(isort_ngp(ip)).gt.0.)then
                       is = is + 1
                       x_sel2(is,1) = pos(isort_ngp(ip),1)
                       x_sel2(is,2) = pos(isort_ngp(ip),2)
                       x_sel2(is,3) = pos(isort_ngp(ip),3)
                    end if
                 end do
              end do
           end do
        ...
        if(Mtotsel .ge. Mmin) then
           ...
           if(search_max_npart_in_sphere) then
              ...
              do while(indx_ngp(l).eq.indc.and.l.lt.npart)
                 xcnew=pos(isort_ngp(l),1)
                 ycnew=pos(isort_ngp(l),2)
                 zcnew=pos(isort_ngp(l),3)
  ...
  if(write_masst_strct)then
     ...
     pos=pos/scale !conversion forward to be improved
     gap=0
     if(write_masst_strct)open(26,file=trim(filestrctfof),form='unformatted')
     write(26) namas
     do i=1,namas 
        gap=gap+masses(i)
        write(26) masses(i)
        write(26) ((pos(allpart(j+gap-masses(i)),k),k=1,3),j=1,masses(i))
        write(26) ((vel(allpart(j+gap-masses(i)),k),k=1,3),j=1,masses(i))
        write(26) (id(allpart(j+gap-masses(i))),j=1,masses(i))
     enddo
     close(26)
     pos=pos*scale !conversion backward to be improved
  endif
