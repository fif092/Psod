program sphericaloverdensity
  ...
  character*200::nomfich,repository
  logical::ok
  ...
  do i=1,ndim
     read(1)tmp
     x(npart+1:npart+npart2,i)=real(tmp)
  end do
  do i=1,ndim
     read(1)tmp
  end do
  read(1)tmp
  mp(npart+1:npart+npart2)=real(tmp)
  if(nstar_tot > 0) then
      allocate(inttmp(1:npart2))
      read(1)inttmp
      read(1)inttmp
      read(1)tmp
      ap(npart+1:npart+npart2)=real(tmp)
     deallocate(inttmp)
  endif
  deallocate(tmp)
  close(1)
 ...
 !on enleve les etoiles
  if(nstar_tot > 0)then
     !mass
     allocate(tmpmp   (1:npart))
     tmpmp=mp
     deallocate(mp)
     allocate(mp(1:npart-nstar_tot))
     idm=0
     do ipart=1,npart
        if (ap(ipart)==0.)then
           idm=idm+1
     ...
     !postion
     allocate(tmpx   (1:npart,1:ndim))
     tmpx=x
     deallocate(x)
     allocate(x(1:npart-nstar_tot,1:ndim))
     idm=0
     do ipart=1,npart
        if (ap(ipart)==0.)then
           idm=idm+1
     ...
     npart=npart-nstar_tot
     deallocate(ap)
  endif
  

  write(*,*)'Found ',npart,idm,' particles (without stars)'
...
!---------------------------------------------------------------------------
  ...
  !!real::frac=0.2 !!! Control when to stop the loop on particles. Added by Yann
  real::frac=1.
  structure = 0
  taillex   = dble(nx)
  tailley   = dble(ny)
...
999 format (I8,1x,I8,5(1x,1pe20.7))
