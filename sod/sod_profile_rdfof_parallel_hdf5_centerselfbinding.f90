...
! rayon_min =0.62d0*scale/nx*factrmin !RYtest (4/3 pi)^(1/3)=0.62
  ...
  do i  = 1,npart
     ...
     indx_ngp(i) = 1+i1+i2*local_nx+i3*local_nx*local_ny
  ...
  do while( (ipart.le.npart_new).and.(dble(count_no_halo)/dble(npart_new).le.frac))
     ...
     if(xsort(isort(ipart)).gt.0.)then !this is to eliminate particles which are already in a halo
        ...
        indc=1+ic+jc*local_nx+kc*local_nx*local_ny
        mtotsel=0.d0
        do i=-ncut,ncut
           ii=i+ic
                       ...
                       if (kk.ge.0.and.kk.lt.local_nz) then
                          ...
                          do ip = imin,imax
                             if(xsort(isort_ngp(ip)).gt.0.)then
                                is = is + 1
                                if(is > Nsel)then
                                   write(*,*)procID,'WARNING: from ',nzonechar,' Increase Nsel in main program',' is=',is,' Nsel=',Nsel
                                         ...
                                         if(xsort(isort_ngp(ip)).gt.0.)then
                                            ...
                                            do jjj=1,npart_sel2
                                               ...
                                               if (distance_new.le.rayon_min**2) then
                                                  ...
                                                  if(do_weight_self_binding_like)abs_e_self_bind_rough=abs_e_self_bind_rough+1.d0/sqrt(distance_new+dx_smoothing_resolmax**2)
  ...
  if(write_masst_strct)then
    ...
    if(procID==0)then
     ...
     if(param%gatherwrite_factor <= 1) then
        if(potential)then
           if(force) then
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, halopartVel, halopartID,halopartFor=halopartFor,halopartPot=halopartPot,inforamses=inforamses)
           else
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, halopartVel, halopartID,halopartPot=halopartPot,inforamses=inforamses)
           endif
        else
           if(force) then
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, halopartVel, halopartID,halopartFor=halopartFor,inforamses=inforamses)
           else
              call h5writehalopart(param, namas, gap, haloMass, haloID, halopartPos, halopartVel, halopartID,inforamses=inforamses)
           endif
        endif
     else
        if(potential)then
            if(force) then
               call mpih5writehalopart(MpiSubCubeWrite,param, namas, gap, haloMass, haloID, halopartPos, halopartVel, halopartID,halopartFor=halopartFor,halopartPot=halopartPot,inforamses=inforamses)
            else
               call mpih5writehalopart(MpiSubCubeWrite,param, namas, gap, haloMass, haloID, halopartPos, halopartVel, halopartID,halopartPot=halopartPot,inforamses=inforamses)
            endif
        else
           if(force) then
              call mpih5writehalopart(MpiSubCubeWrite,param, namas, gap, haloMass, haloID, halopartPos, halopartVel, halopartID,halopartFor=halopartFor,inforamses=inforamses)
           else
              call mpih5writehalopart(MpiSubCubeWrite,param, namas, gap, haloMass, haloID, halopartPos, halopartVel, halopartID,inforamses=inforamses)
           endif
        endif
     endif
     ...
     call  mpih5writehalomass(MPI_COMM_WORLD, param,haloNB_all, namas,nh, haloMass, halocomPos, halocomVel, haloID, haloRadius, haloSubHaloNB,inforamses=inforamses)
