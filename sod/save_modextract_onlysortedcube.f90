...
!    Call hdf5_init()
    ...
    Do i=1,3
       globindexextract(2*i-1) = floor((center(i) - 0.5*dimensions(i)) * ncglob, kind=4) + 1
       globindexextract(2*i)   = floor((center(i) + 0.5*dimensions(i)) * ncglob, kind=4) + 1
    ...
    Do i=globindexextract(1), globindexextract(2)
       imod=mod(i+ncglob-1,ncglob)+1
       Do j=globindexextract(3), globindexextract(4)
          jmod=mod(j+ncglob-1,ncglob)+1
          Do k=globindexextract(5), globindexextract(6)
             kmod=mod(k+ncglob-1,ncglob)+1
             ifile = coord2file(imod,jmod,kmod)
             ngroupperfile(ifile) = ngroupperfile(ifile)+1
             locindex2extract(2,ind) = ifile
             iloc = imod - globindexcube(1,ifile) + 1
             jloc = jmod - globindexcube(3,ifile) + 1
             kloc = kmod - globindexcube(5,ifile) + 1
